...
public class JobExeLogIn {
    ...
    public void onOpen(Session session, @PathParam("unit") String unit, @PathParam("name") String name) {
        SessionManager.getInstance(JobExeSession.class).getSession(new UnitNamePK(unit, name)).setPublisher(session);
    }
    ...
    public void onMessage(String message, Session session, @PathParam("unit") String unit, @PathParam("name") String name) {
        SessionManager.getInstance(JobExeSession.class).getSession(new UnitNamePK(unit, name)).publish(message);
    }

    @OnError
    public void onError(Throwable t) {
        Logger.getLogger(JobExeLogIn.class.getName()).log(Level.SEVERE, null, t);
    }
